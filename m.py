...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Yha aake paise de pahle @itxx_abby."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP PAID USERS:- @ITXX_ABBY"
    bot.reply_to(message, response)
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "bhen ke lode aukaat me ."
...
Buy From :- @itxx_abby
Official Channel :- bnauga tab btauga 
...
def welcome_start(message):
    ...
    response = f'''Aao mere sher attack marna h message kr @itxx_abby.
BUY :- @itxx_abby'''
    bot.reply_to(message, response)
...
4. IF You use DDOS Play normal match before going ofline !!'''
    bot.reply_to(message, response)
